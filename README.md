...
1. **数据库密码**：修改 `DBUtil.java` 中的数据库密码，不要使用默认的 'root'/'root'
2. **启用SSL**：在生产环境中启用数据库SSL连接
3. **密码加密**：当前演示版本使用明文密码比较，生产环境应使用 BCrypt 或 Argon2 进行密码哈希
4. **上传目录权限**：确保上传目录有适当的文件系统权限
5. **环境变量**：将敏感配置（数据库凭据、密钥等）移至环境变量或配置文件
